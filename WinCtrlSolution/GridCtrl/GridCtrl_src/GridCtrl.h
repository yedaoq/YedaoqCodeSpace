...
// Structures
///////////////////////////////////////////////////////////////////////////////////
typedef struct tagCELLCONTEXT
{
	int   iRow;
	int   iColumn;

	tagCELLCONTEXT(int row, int col)
...
typedef std::vector<CGridColumn> ColumnVct;
...
public:
    ...
    BOOL   SetItemTextDrawFormat(int nRow, int nCol, UINT nFormat);
    UINT   GetItemTextDrawFormat(int nRow, int nCol) const;
	...
	int SetCellEx(int nRow, int nCol, CGridCellBase *cell);
...
public:
    int  InsertColumn(LPCTSTR strHeading, UINT nFormat = DT_CENTER|DT_VCENTER|DT_SINGLELINE,
                      int nColumn = -1);
	int  InsertColumn(const CGridColumn& col, UINT nFormat = DT_CENTER|DT_VCENTER|DT_SINGLELINE,
						int nColumn = -1);
...
inline CGridCellBase* CGridCtrl::GetCell(int nRow, int nCol) const
{
    ...
    if (GetVirtualMode())
    {
        ...
        gvdi.item.nState  = 0;
        gvdi.item.nFormat = pCell->GetTextDrawFormat();
        ...
        cell.SetTextDrawFormat(gvdi.item.nFormat);
