...
BOOL CGridCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle)
{
    ...
    for (i = 0; i < m_nCols; i++)
	{
		...
		if(m_arColumns[i].GetWidth() == -1)
		{
			m_arColumns[i].SetWidth(m_cellDefault.GetWidth());
		}
	}
...
void CGridCtrl::ClearCells(CCellRange Selection)
{
    for (int row = Selection.GetMinRow(); row <= Selection.GetMaxRow(); row++)
    {
        for (int col = Selection.GetMinCol(); col <= Selection.GetMaxCol(); col++)
        {
            ...
            if ( m_arRowHeights[row] > 0 && m_arColumns[col].GetWidth() > 0 )
            {
    ...
    TRY 
    {
        ...
        if (addedCols > 0)
        {
            ...
            for (col = startCol; col < nCols; col++)
				...
				m_arColumns[col].SetWidth(m_cellFixedColDef.GetWidth());
...
int CGridCtrl::InsertColumn(LPCTSTR strHeading,
                            ...
                            int nColumn  /* = -1 */)
{
	return InsertColumn(CGridColumn(strHeading), nFormat, nColumn);
  
    //return nColumn;
}

int CGridCtrl::InsertColumn(const CGridColumn& columnInfo, 
							UINT nFormat/* = DT_CENTER|DT_VCENTER|DT_SINGLELINE*/,
							int nColumn/* = -1*/)
{
	...
	TRY
	{
		...
		{
			nColumn = m_nCols;
			m_arColumns.push_back(columnInfo);  //m_arColWidths.Add(0);
			if (!GetVirtualMode())
			{
				for (int row = 0; row < m_nRows; row++)
				{
		...
		{
			m_arColumns.insert(m_arColumns.begin() + nColumn, 1, columnInfo);  //m_arColWidths.InsertAt(nColumn, (UINT)0);
			if (!GetVirtualMode())
			{
				for (int row = 0; row < m_nRows; row++) 
				{
	...
	// Initialise column data
	SetItemText(0, nColumn, columnInfo.GetTitle().c_str());
	for (int row = 0; row < m_nRows; row++) 
		SetItemTextDrawFormat(row, nColumn, nFormat);
...
// Init 
int CGridCtrl::SetCellEx(int nRow, int nCol, CGridCellBase *cell)
{
	...
	SetCell(nRow, nCol, cell);

	if (nCol < m_nFixedCols)
		cell->SetState(cell->GetState() | GVIS_FIXED | GVIS_FIXEDCOL);
	if (nRow < m_nFixedRows)
		cell->SetState(cell->GetState() | GVIS_FIXED | GVIS_FIXEDROW);
...
/*virtual*/ CGridCellBase* CGridCtrl::CreateCell(int nRow, int nCol)
{
    ...
    pCell->SetTextDrawFormat(pCell->GetDefaultCell()->GetTextDrawFormat());
...
BOOL CGridCtrl::DeleteColumn(int nColumn)
{
    ...
    m_arColumns.erase(m_arColumns.begin() + nColumn);  //m_arColWidths.RemoveAt(nColumn);
    m_nCols--;
...
BOOL CGridCtrl::DeleteAllItems()
{
    ...
    m_arColumns.clear();  //m_arColWidths.RemoveAll();
...
BOOL CGridCtrl::SetItem(const GV_ITEM* pItem)
{
    ...
    if (pItem->mask & GVIF_FORMAT)
        pCell->SetTextDrawFormat(pItem->nFormat);
...
BOOL CGridCtrl::GetItem(GV_ITEM* pItem)
{
    ...
    if (pItem->mask & GVIF_FORMAT)
        pItem->nFormat = pCell->GetTextDrawFormat();
    if (pItem->mask & GVIF_BKCLR)
...
BOOL CGridCtrl::SetItemTextDrawFormat(int nRow, int nCol, UINT nFormat)
{
    if (GetVirtualMode())
        return FALSE;
    ...
    pCell->SetTextDrawFormat(nFormat);
    return TRUE;
}

UINT CGridCtrl::GetItemTextDrawFormat(int nRow, int nCol) const
{
    CGridCellBase* pCell = GetCell(nRow, nCol);
    ASSERT(pCell);
    if (!pCell)
    ...
    return pCell->GetTextDrawFormat();
}
...
long CGridCtrl::GetVirtualWidth() const
{
    ...
    for (int i = 0; i < iColCount; i++)
         lVirtualWidth += m_arColumns[i].GetWidth(); //lVirtualWidth += m_arColWidths[i];
...
int CGridCtrl::GetColumnWidth(int nCol) const
{
    ...
    return m_arColumns[m_arColOrder[nCol]].GetWidth();  //m_arColWidths[m_arColOrder[nCol]];
}
                               ...
                               BOOL bResetScroll /*=TRUE*/)
{
    ...
    m_arColumns[nCol].SetWidth(nWidth);  //m_arColWidths[nCol] = nWidth;
...
void CGridCtrl::AutoSize(UINT nAutoSizeStyle /*=GVS_DEFAULT*/)
{
    ...
    for (nCol = 0; nCol < nNumColumns; nCol++)
    {
        ...
        if( GetColumnWidth( nCol) > 0 )
        {
            ...
            m_arColumns[nCol].SetWidth(0);  //m_arColWidths[nCol] = 0;
            for (nRow = nStartRow; nRow <= nEndRow; nRow++)
            {
                if( GetRowHeight( nRow) > 0 )
                {
                    ...
                    if (pCell)
                        size = pCell->GetCellExtent(pDC);
                    if (size.cx >(int) /*m_arColWidths*/m_arColumns[nCol].GetWidth())
                        m_arColumns[nCol].SetWidth(size.cx);  //m_arColWidths[nCol] = size.cx;
...
void CGridCtrl::ExpandColumnsToFit(BOOL bExpandFixed /*=TRUE*/)
{
    ...
    {
        if (/*m_arColWidths*/m_arColumns[col].GetWidth() > 0)
            nNumColumnsAffected++;
    }
    ...
    {
        if (/*m_arColWidths*/m_arColumns[col].GetWidth() > 0)
            /*m_arColWidths*/m_arColumns[col].SetWidth(m_arColumns[col].GetWidth() + nColumnAdjustment); // += nColumnAdjustment;    
    }
    ...
    if (nDifference > 0)
    {
        ...
        {
            if (/*m_arColWidths*/m_arColumns[col].GetWidth() > 0)
                m_arColumns[col].SetWidth(m_arColumns[col].GetWidth() + 1);  //m_arColWidths[col] += 1;
        }
    }
    else 
    {
...
void CGridCtrl::ExpandLastColumn()
{
    ...
    while (/*m_arColWidths*/m_arColumns[nLastColumn].GetWidth() <= 0)
        nLastColumn--;
    ...
    if (nDifference > 0)
    {
        ...
        m_arColumns[nLastColumn].SetWidth(m_arColumns[nLastColumn].GetWidth() + nDifference);  //m_arColWidths[ nLastColumn ] += nDifference;
        Refresh();
    }
