...
void CGridCell::Reset()
{
    ...
    m_nTextDrawFormat = (DWORD)-1;       // Use default from CGridDefaultCell
...
DWORD CGridCell::GetTextDrawFormat() const
{
    if (m_nTextDrawFormat == (DWORD)-1)
    {
        CGridDefaultCell *pDefaultCell = (CGridDefaultCell*) GetDefaultCell();
        if (!pDefaultCell)
            return 0;

        return pDefaultCell->GetTextDrawFormat();
    }

    return m_nTextDrawFormat; 
}
...
BOOL CGridCell::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
	...
	{   
		...
		if (GetTextDrawFormat() & DT_RIGHT) 
			dwStyle = ES_RIGHT;
		else if (GetTextDrawFormat() & DT_CENTER) 
			dwStyle = ES_CENTER;
...
{
#ifdef _WIN32_WCE
    m_nTextDrawFormat = DT_LEFT|DT_VCENTER|DT_SINGLELINE|DT_NOPREFIX;
#else
    m_nTextDrawFormat = DT_LEFT|DT_VCENTER|DT_SINGLELINE|DT_NOPREFIX | DT_END_ELLIPSIS;
#endif
