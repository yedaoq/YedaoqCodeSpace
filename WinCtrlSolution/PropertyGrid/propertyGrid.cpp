...
#define MAKE_PRECT(left, top, right, bottom) (&(RECT rect = { (left), (top), (right), (bottom) }))
...
BOOL ToolTip_AddTool(HWND hWnd, LPTOOLINFO lpToolInfo)
{
	return (BOOL)::SendMessage(hWnd, TTM_ADDTOOL, 0, (LPARAM)lpToolInfo);
}

BOOL ToolTip_EnumTools(HWND hWnd, UINT nTool, LPTOOLINFO lpToolInfo)
{
	return (BOOL)::SendMessage(hWnd, TTM_ENUMTOOLS, nTool, (LPARAM)lpToolInfo);
}

void ToolTip_UpdateTipText(HWND hWnd, LPTOOLINFO lpToolInfo)
{
	::SendMessage(hWnd, TTM_UPDATETIPTEXT, 0, (LPARAM)lpToolInfo);
}

void ToolTip_SetToolInfo(HWND hWnd, LPTOOLINFO lpToolInfo)
{
	::SendMessage(hWnd, TTM_SETTOOLINFO, 0, (LPARAM)lpToolInfo);
}
...
static VOID LogFontItem_FromString(LPPROPGRIDFONTITEM lpLogFontItem, LPTSTR lpszFont)
{
    ...
    _stscanf(lpszFont,
        _T( "Height: %d Width: %d Escapement: %d Orientation:  %d Weight: %d Italic: %hhd Underline: %hhd StrikeOut: %hhd CharSet: %hhd OutPrecision: %hhd ClipPrecision: %hhd Quality: %hhd PitchAndFamily: %hhd ")
#ifdef _UNICODE
        _T("FaceName: %32l[^\r\n] ")
#else
        _T("FaceName: %32[^\r\n] ")
...
static LPITEMIDLIST ConvertPathToLpItemIdList(LPTSTR pszPath)
{
    ...
    {
		//IShellFolder_ParseDisplayName(pDesktopFolder, NULL, NULL, pszPath, &chEaten, &pidl, &dwAttributes );
        pDesktopFolder->lpVtbl->ParseDisplayName(pDesktopFolder, NULL, NULL, pszPath, &chEaten, &pidl, &dwAttributes);
...
static VOID Grid_OnSize(HWND hwnd, UINT state, INT cx, INT cy)
{
    ...
    if(NULL != g_lpInst->hwndToolTip)//Resize ToolTips
    {
        ...
        /*ToolTip_NewToolRect*/ToolTip_SetToolInfo(g_lpInst->hwndToolTip, &ti);
    }
    if (NULL != g_lpInst->lpCurrent)
    {
...
static HBRUSH Grid_OnCtlColorStatic(HWND hwnd, HDC hdc, HWND hwndChild, INT type)
{
    ...
    if (NULL != g_lpInst->hwndPropDesc)
    {
        ...
        HDC hdc = GetDC(hwnd);
		RECT rect = {0, g_lpInst->iVDivider - 2, WIDTH(rc), g_lpInst->iVDivider };
...
static VOID Grid_OnDrawItem(HWND hwnd, const DRAWITEMSTRUCT *lpDIS)
{
    ...
    if (PIT_CATALOG == pItem->iItemType)
    {
		...
		RECT rect = {rectCatPart1.left + 6, rectCatPart1.top + 3,rectCatPart1.right - 6, rectCatPart1.bottom + 3};
        DrawText(lpDIS->hDC, pItem->lpszCatalog, _tcslen(pItem->lpszCatalog),&rect,DT_NOCLIP | DT_LEFT | DT_SINGLELINE);
    }
    else
    {
		...
		RECT rect = {rectPart1.left + 3, rectPart1.top + 3, rectPart1.right - 3,rectPart1.bottom + 3};
        DrawText(lpDIS->hDC, pItem->lpszPropName, _tcslen(pItem->lpszPropName), &rect, DT_NOCLIP | DT_LEFT | DT_SINGLELINE);
    ...
    if (PIT_CHECK != pItem->iItemType)
    {
        ...
        if (PIT_FONT == pItem->iItemType)
        {
            ...
            if (0 < PointSize)
            {
				...
				RECT rect = {rectPart3.left + 3, rectPart3.top + 3,	rectPart3.right + 3, rectPart3.bottom + 3};
                DrawText(lpDIS->hDC, buf, _tcslen(buf), &rect, DT_NOCLIP | DT_LEFT | DT_SINGLELINE);
        ...
        else
        {
			...
			RECT rect = {rectPart3.left + 3, rectPart3.top + 3,	rectPart3.right + 3, rectPart3.bottom + 3};
            DrawText(lpDIS->hDC, pItem->lpszCurValue, _tcslen(pItem->lpszCurValue), &rect, DT_NOCLIP | DT_LEFT | DT_SINGLELINE);
        }
    }
