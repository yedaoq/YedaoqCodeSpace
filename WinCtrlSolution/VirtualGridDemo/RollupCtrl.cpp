...
{
	return  CWnd::Create(m_strMyClass, TEXT("RollupCtrl"), dwStyle, rect, pParentWnd, nID);
}
...
int CRollupCtrl::_InsertPage(LPCTSTR caption, CDialog* pwndTemplate, int idx, BOOL bAutoDestroyTpl)
{
	...
	groupbox->Create(TEXT(""), WS_CHILD|BS_GROUPBOX, r, this, 0 );
...
void CRollupCtrl::OnContextMenu(CWnd* pWnd, CPoint point) 
{
	...
	{
		menu.AppendMenu(MF_STRING,		RC_IDM_EXPANDALL,	TEXT("Expand all"));
		menu.AppendMenu(MF_STRING,		RC_IDM_COLLAPSEALL,	TEXT("Collapse all"));
		menu.AppendMenu(MF_SEPARATOR,	0,					TEXT(""));
